struct zhu_liu{
	static const int MAXN=1100,MAXM=1005005;
	struct node{
		int u,v;
		LL w,tag;
		node *l,*r;
		node(int u=0,int v=0,LL w=0):u(u),v(v),w(w),tag(0),l(0),r(0){}
		void down(){
			w+=tag;
			if(l)l->tag+=tag;
			if(r)r->tag+=tag;
	...
	int st[MAXN*2],id[MAXN*2],m,from[MAXN*2];
	void init(int n){
		for(int i=1;i<=n;++i){
			pq[i]=E[i]=0;
			st[i]=id[i]=i;
	...
	node *merge(node *a,node *b){//skew heap
		...
		if(b->w==a->w&&b->v<a->v)return merge(b,a);//
		swap(a->l,a->r);
		a->l=merge(b,a->l);
		return a;
	}
